...
opts = (resolution=2.5, )
boundingbox = (bottom=41.0, right=-69.0, left=-84.00, top=48.0)
...
land = replace(coarsen(water, mean, (5, 5)).<100, false => nothing)
```

```{julia}
fig = Figure(; resolution=(900, 500))
ax = Axis(fig[1,1]; xlabel="Longitude", ylabel="Latitude", aspect=DataAspect())
hm = heatmap!(ax, mask(land, temperature), colormap=Reverse(:dense))
...
```{julia}
Hlac = taxon("Procyon lotor"; strict=false)
query = [
