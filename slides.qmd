...
opts = (;)
boundingbox = (left=8.412,bottom=41.325,right=9.662,top=43.060)
...
#| echo: true
water = 
    SimpleSDMPredictor(RasterData(EarthEnv, LandCover), layer=12; opts..., boundingbox...) .< 100
...
#| echo: true
#| output: false
critter = taxon("Sitta whiteheadi"; strict=false)
```
...
hm = heatmap!(ax, temperature, colormap=cgrad(incandescent))
...
sil_pos = ([8.6], [42.9])
scatter!(ax, sil_pos...; marker = silhouette, markersize = silhouette_size, color=:white)
current_figure()
```
...
#| echo: true
#| output: false
background = pseudoabsencemask(WithinRadius, presence_layer; distance = 70.0)
buffer = pseudoabsencemask(WithinRadius, presence_layer; distance = 10.0)
...
fig = Figure(; resolution=(900, 500))
ax = Axis(fig[1,1]; xlabel="Longitude", ylabel="Latitude", aspect=DataAspect())
hm = heatmap!(ax, prediction, colormap=cgrad(incandescent), colorrange=(0., 1.))
...
fig = Figure(; resolution=(900, 500))
ax = Axis(fig[1,1]; xlabel="Longitude", ylabel="Latitude", aspect=DataAspect())
hm = heatmap!(ax, entropy.(prediction), colormap=cgrad(incandescent), colorrange=(0., 1.))
Colorbar(fig[1,2], hm; tellheight=false)
current_figure()
```
